...
db.run("CREATE TABLE IF NOT EXISTS sensordata (unixtime INTEGER, id INTEGER, temp REAL, relhum REAL, abshum REAL)")
...
app.use(async ctx => {
    if (ctx.url == "/log") {
      ...
      if (nhoursQuery && mode != "dates") fromDate = dayjs().add(-parseInt(nhoursQuery),"hours")
      ...
      console.log(toDate)
      let dbdata = await getSensorData(fromDate.unix()*1000, toDate.unix()*1000)
...
function calAbsHum(temp, hum) {
  let abshum = 6.112 * Math.exp(17.67*temp/(temp+243.5)) * hum * 2.1674 / (273.15 + temp)
