...
function loadData() {
  ...
  datapromise.then(data => setupChart(queryParams.min, queryParams.endDate, data, 'tempChart', 'Temperatur', names, '°C', 2, ['#E83B27','#2f2c78','#e687e4']))
  datapromise.then(data => setupChart(queryParams.min, queryParams.endDate, data, 'relHumChart', 'relative Luftfeuchtigkeit', names, '%', 3, ['#E83B27','#2f2c78','#e687e4']))
  datapromise.then(data => setupChart(queryParams.min, queryParams.endDate, data, 'absHumChart', 'absolute Luftfeuchtigkeit', names, 'g/m³', 4, ['#E83B27','#2f2c78','#e687e4']))

  datapromise.then(data => window.data = data)
}

async function setupChart(min, endDate, data, chartid, title, names, unit, dataindex, colors) {
  ...
  let chartdata = {
    type: 'line',
    data: {
      datasets: colors.map( (color, index) => { return {
        label: `#${index+1} (${names[index]})`,
        data: data.filter(r => r[1] == index+1).filter(r => min?r[0] >= min:true).map(r => {return {x: r[0],y:r[dataindex]<200?r[dataindex]:undefined}}),
  ...
  if(title == 'Temperatur') {
    ...
    chartdata.data.datasets.push ({
      ...
      data: data.filter(r => r[1] == 3).filter(r => min?r[0] >= min:true).map(r => {return {x: r[0],y:r[2]<200?dewPoint(r[2], r[3]):undefined}}),
      backgroundColor: color,
      fill: false,
      borderColor: color,
      borderWidth: 3,
